...
ProgressFunc<CMAParameters<GenoPheno<pwqBoundStrategy>>,CMASolutions> progFunc = [](const CMAParameters<GenoPheno<pwqBoundStrategy>> &cmaparams, const CMASolutions &cmasols)
{
	...
	fprintf(fp, "    Best Candidate: %.15lf", sph[0]);
	printf("    Best Candidate: %.15lf", sph[0]);
	for (int i = 1; i < CFFCT; i++)
	{
		fprintf(fp, ", %.15lf", sph[i]);
		printf(", %.15lf", sph[i]);
	}
...
int main(void)
{
	...
	std::vector<double> x0(dim, M_PI_4);
...
#endif
#ifndef SCORECFF
	cmaparams.set_fixed_p(7, M_PI_2);
	cmaparams.set_fixed_p(8, M_PI_2);
	cmaparams.set_fixed_p(9, M_PI_2);
#endif
